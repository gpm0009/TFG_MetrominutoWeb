...
El proyecto se ha llevado a cabo por un único desarrollador contratado a tiempo parcial durante 9 meses, considerando un salario  bruto anual de \EUR{21000} (ver tabla~\ref{tab:personal})


\tablaSmallSinColores{Costes de personal.}{p{6cm} p{3cm} p{3cm}}{personal}{
	\multicolumn{1}{p{4.5cm}}{\textbf{Concepto}} & \textbf{Devengos{}} & \textbf{Deducciones{}}\\
}
{
	Salario base & \EUR{1500}  \\
	Cotización contingencias comunes (4,70\%) & & \EUR{70,50} \\\texttt{}
	Cotización formación (0,10 \%) & & \EUR{1,50} \\
	Cotización desempleo (1,60 \%) & & \EUR{24} \\
	Retención IRPF (10 \%) & & \EUR{150,00} \\\hline
	\textbf{Total mes} & \EUR{1254}\\
	\textbf{Total 9 meses} & \EUR{11286}\\\hline
}
...
{
	Ordenador portátil & \EUR{900} & \EUR{15} \\
	Microsoft Windows 10 Home 64Bits OEM & \EUR{118,76} & \EUR{1,98} \\\hline
	\textbf{Coste total (9 meses)} & \multicolumn{1}{r}{\EUR{152,82}}\\\hline
}
...
\tablaSmallSinColores{Costes de software.}{p{5cm} p{4cm} p{2.15cm}}{software}{
	\multicolumn{1}{p{4.5cm}}{\textbf{Concepto}} & \textbf{Coste{}}\\
}
{
	PyCharm Professional~\cite{pycharm-price}  & \EUR{89,00}$\backslash$año \\\hline
	\textbf{Coste total (9 meses)} & \multicolumn{1}{r}{\EUR{66,65}}\\\hline
}
...
}
{
	...
	Nivel & Básico &  \\
	Instancia & B1: Núcleos: 1, 1.75 GB de RAM, 10 GB de almacenamiento & \EUR{0,063}$\backslash$hora \\
	Número de instancias & Dos &  \\
	Tiempo & Un mes &  \\
	Soporte & Estándar &  \EUR{84,33}$\backslash$hora\\
	Programa de licencias & Contrato de Microsoft Online Services &  \\\hline
	\textbf{Coste total (4 meses)}  & & \multicolumn{1}{r}{\EUR{713,76}}\\\hline
}
...
\tablaSmallSinColores{Coste Google API}{p{6.4cm} p{2.15cm} p{8cm}}{google-api-costes}{
	\multicolumn{1}{p{4.5cm}}{\textbf{Concepto}} & \textbf{Coste{}$\backslash$1000 solicitudes}\\
}
{
	API Maps Javascript & \EUR{6,17} \\
	API Distance Matrix & \EUR{4,41} \\
	Geocoding & \EUR{4,41} \\
	API Directions & \EUR{4,41} \\
	Autocomplete -- Per Request & \EUR{2,5} \\
	Places Details & \EUR{15} \\\hline
}

Para los costes de los servicios de Firebase se ha consultado su lista de precios~\cite{firebase-price}, aunque si las peticiones son menores de 10.000$\backslash$mes podríamos emplear el plan gratuito.
